...
#     def add_response_to_map(self, sdir, pixmap):
#         """Add the PSF response centered on `sdir` to an existing HEALPix map `pixmap`."""
#         cpix, vpix = self.evaluate(sdir)
#         pixmap[cpix] += vpix
    
#     def plot_psf_map(self, sdir, **kwargs):
...
#         zfig = ZEAfigure(sdir, **kw)
#         zfig.imshow(np.log10(pixmap), )
#         zfig.colorbar(label='log(PSF value)')
#         zfig.scatter(sdir, c='red', marker='x', label='Center')
#         zfig.legend()
#         zfig.show()
...
#             sdir = SkyCoord(*sdir, unit='deg', frame='galactic')
#         self.plot_response(sdir)
#         self.plot_psf_map(sdir)
