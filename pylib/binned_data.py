...
class BandList(object):
    ...
    def __init__(self, bands_hdu):
        ...
        if 'COUNT' in bands_hdu.columns.names:
            ...
            self.pixelcnt = np.array([0 if n<0 else n for n in bands_hdu.data.field('COUNT')],int)
        else:
            self.pixelcnt=None
...
class Pixels(object):
    ...
    def _make_counter(self):
        ...
        if self.counter is None:
            ...
            self.counter = Counter(dict(list(zip(keys, self.cnt))))
        return self.counter
    ...
    def _decode_counter(self):
        ...
        items = np.array(list(self.counter.items()), int)
        keys = items[:,0]
    ...
    def __getitem__(self, channel):
        ...
        if not self._sorted:
            ...
            self.lookup = dict(list(zip(channels,list(zip(indexchan[:-1], indexchan[1:])))))
            self._sorted = True
        try:
            a,b = self.lookup[channel]
            return list(zip(self.pix[a:b], self.cnt[a:b]))
        except KeyError:
...
class BinFile(object):
    ...
    def __init__(self, filenames, outfile=None, adding=False, quiet=True):
        ...
        for i,filename in enumerate(filenames):
            if i==0: # first one: will add others, if any to this one
                ...
                if not quiet: print(self.pixels.__repr__(), end=' ')
            else:
    ...
    def generate_ccube_files(self, path, roi_index, channels=list(range(8)), overwrite=False):
...
class KerrDataFile:
    ...
    def skymap_hdu(self):
        ...
        for n,v in zip(nocc, et):
            chn = np.append(chn, np.full(n, v, dtype=np.uint32))
...
#             if not quiet:
#                 print('{:3} {:3} {:6}'.format( band.ie, band.event_type, band.nside), end=' ') 
