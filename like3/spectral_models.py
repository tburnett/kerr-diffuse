...
class Model(object):
	...
	"""        
        ...
        if 'mappers' in kwargs:
            self.mappers = kwargs.pop('mappers')
        else:
            self.mappers = copy.deepcopy(self.default_mappers)

        try:
            for k,v in list(self.default_extra_params.items()): 
                setattr(self,k,v)
        except: pass

        for k,v in list(self.default_extra_attrs.items()):
            setattr(self,k,v)
    ...
    def getp(self, i, internal=False):
        """ Get external value for parameter # i 
            ...
            """
        if i in list(self.default_extra_params.keys()):
    ...
    def get_all_parameters(self, internal=False):
        ...
        if internal:
            ...
            return np.asarray(list(map(self.getp,np.arange(self.npar))))
    ...
    def set_limits_gtlike(self, i, lower, upper, scale=-1, strict=False):
        ...
        i=self.name_mapper(i)
        m = self.gtlike['topointlike'][i]
        lower, upper, scale = list(map(m,[lower, upper, scale]))
    ...
    def set_parameters(self,new_vals):
        ...
        assert(len(new_vals)==(self.free).sum()), 'called with %d, expected %d free parameters' % (len(new_vals), (self.free).sum())
    ...
    def __str__(self,absolute=False, indent='', no_flux=True):
        ...
        if not self.background and hi_p is not None:
            ...
            pnames = self.param_names + ['Ph. Flux','En. Flux'] if not no_flux else self.param_names
        else:
    ...
    def flux_relunc(self, energy):
        ...
        def unc(energy):
            ...
            return np.sqrt(t)/self(energy) if t>0 else t
        return np.array(list(map(unc,energy))) if hasattr(energy, '__iter__') else unc(energy)
    ...
    def get_pointlike_name(cls, name):
        ...
        if name in list(epn.values()):
            return list(epn.keys())[list(epn.values()).index(name)]
    ...
    def gtlike_param_names(cls):
        ...
        """
        return cls.gtlike['param_names'] + list(cls.gtlike['extra_param_names'].values())
    ...
    def from_gtlike(cls, **kwargs):
        ...
        model=cls(**kwargs)
        for gn,v in list(gtlike_params.items()):
...
class PLSuperExpCutoff4(Model):
    ...
    def __call__(self,e):
        ...
        try:
            ...
            print(f"""Caught a RuntimeWarning: {w}, with parameters: n0={n0:.2e}, 
                  gamma={gamma:.2f}, d={d:.2f}, b={b:.2f}""")
            return 1e-20
...
class CompositeModel(Model):
    ...
    def default_limits(self):
        ...
        return dict(reduce(operator.add,[list(i.default_limits.items()) for i in self.models]))
    ...
    def setp(self, i, *args, **kwargs):
        if i in list(self.default_extra_params.keys()) + list(self.default_extra_attrs.keys()):
