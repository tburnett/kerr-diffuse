...
class ROI(views.LikelihoodViews):
    ...
    def find_associations(self, source_name=None, classes='all_but_gammas', srcid='srcid', quiet=True):
        ...
        if source_name=='all':
            sources = [s for s in self.sources if np.any(s.model.free) and not s.isglobal and not s.isextended]
    ...
    def fit_curvature(self, source_name=None, ignore_exception=True,):
        ...
        if source_name=='ALL':
            list(map(fit_one, self.free_sources))
        else:
    ...
    def correlation_info(self, corr_min=0.2, **fit_kw):
        ...
        isel = [i for i in range(len(pnames)) if pnames[i].endswith('_Norm')]
        ...
        sn = [x.split('_')[0] for x in pnames[isel]]; sn
        corr_src = []
