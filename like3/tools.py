...
def ufunc_decorator(f): # this adapts a bound function
    def new_ufunc(self, par):
        return np.array([f(self,x) for x in par]) if hasattr(par, '__iter__')  else f(self,par)
    return new_ufunc
...
def parse_jname(name):
    ...
    return [float(Angle(a, unit=u.deg).to_string(decimal=True)) for a in (ra,dec)]
...
def find_close(A,B):
    ...
    def skydir(df):
        ...
        return list(map(SkyDir, df.ra.values.astype(float), df.dec.values.astype(float)))
    def mindist(a):
        d = list(map(a.difference, skydir(B)))
        n = np.argmin(d)
        return (B.index[n], np.degrees(d[n]))

    return pd.DataFrame( list(map(mindist, skydir(A))),
