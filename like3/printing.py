...
def print_summary(roi, sdir=None, galactic=False, maxdist=5, title=None, print_all_ts=False, no_global=True, free_only=True):
    ...
    local_sources =  [s for s in roi.sources[:] if s.skydir is not None]
    local_sources.sort( key=lambda s: s.skydir.difference(center))
    global_sources = [s for s in roi.sources[:] if s.skydir is None] if not no_global else []
    ...
    for ps in local_sources:
        ...
        npar = len(par)
        index_order = list(range(1,npar)) if model.name!='LogParabola' else (1,3,2)
        ...
        freeflag = list(map(makefreeflag, model.free, sigpar))
    ...
    for source in global_sources:
        ...
        n= len(par)
        freeflag = list(map(makefreeflag, source.spectral_model.free, sigpar))
        fmt ='%-22s' 
